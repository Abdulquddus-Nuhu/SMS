...
namespace Module.Access.Seeding
{
    public class SeedIdentity
    {
        public static async Task SeedAsync(UserManager<Persona>? userManager, RoleManager<Role>? roleManager)
        {
            ...
            var superAdminEmail = DefaultValues.ROOT_ADMIN_EMAIL;
            if (await userManager.FindByEmailAsync(superAdminEmail) is null)
            {
                var superAdmin = new Persona()
                {
                    ...
                    PhoneNumber = DefaultValues.ROOT_ADMIN_PHONENUMBER,
                ...
                var result = await userManager.CreateAsync(superAdmin, DefaultValues.ROOT_DEFAULT_PASS);
                if (result.Succeeded)
                {
